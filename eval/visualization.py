...
def save_crops(image, boxes, save_dir, gt=False):
    crops = []
    for i, box in enumerate(boxes):
        crop = image.crop(box)
        crops.append(crop)
...
def save_pdf(save_dir, image, image_with_gt, image_with_zoom, gt_crops, zoom_crops, image_name, question, answer, pred_ans, acc, conversation):
    pdf_path = os.path.join(save_dir, "summary.pdf")
...
def process_sample(image_name, data, root_path, save_root):
    ...
    save_name = image_name.replace(".jpg", f"_{status}")
    ...
    # Save gt crops
    gt_crops = save_crops(image, gt_boxes, save_dir, gt=True)

    # Save zoom-in crops
    zoom_crops = save_crops(image, zoom_boxes, save_dir, gt=False)
    ...
    # save PDF
    save_pdf(save_dir, image, image_with_gt, image_with_zoom, gt_crops, zoom_crops, image_name, question, answer, pred_ans, acc, conversation)

def main():
    root_path = '/cluster/home3/zhaoyutian/datasets/VstarBench'
    json_path = '/cluster/home3/zhaoyutian/code/deepeyes/DeepEyes/results/Vstar_result/deepeyes/result_direct_attributes_deepeyes_acc.jsonl'
    save_root = '/cluster/home3/zhaoyutian/code/deepeyes/DeepEyes/results/Vstar_result/deepeyes/visualization'
    ...
    for image_name in image_list:
        try:
            ...
            process_sample(image_name, data, root_path, save_root)
            success_count += 1
